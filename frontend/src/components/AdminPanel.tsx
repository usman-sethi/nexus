            ...
            {requests.map((req) => (
              <div key={req.id} className="p-6 flex flex-col lg:flex-row gap-6 hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  {req.status === VerificationStatus.APPROVED && (
                    <div className="text-xs text-slate-500 flex items-center gap-1 bg-emerald-50 p-2.5 rounded-lg border border-emerald-100">
                      <Award className="h-4 w-4 shrink-0 text-emerald-700" /> Verified badge credentials added successfully. Representative and portfolio indexed into campus reputation engine.
                    </div>
                  )}
                </div>
              </div>
