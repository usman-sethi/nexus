...
import { StudentProfile, PortfolioItem, UserSession, VerificationStatus } from "../types";
import { calculateCRS } from "../data";
import { Award, Briefcase, Github, Globe, Plus, Trash2, Sparkles, RefreshCw, Star, ArrowUpRight, MessageSquare, Check, ShieldCheck, ShieldAlert, FileText, CheckCircle2 } from "lucide-react";
  ...
  const handleAddPortfolio = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPfGithub("");
        setPfLive("");
        // Reload profile to reflect immediate CRS scores recalculation
        fetchProfile();
      }
    } catch (e) {
      console.error(e);
      ...
      <div className="relative bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col md:flex-row gap-6 items-start md:items-center">
        <div className="relative shrink-0">
          ...
          {profile.verificationStatus === VerificationStatus.APPROVED && (
            <span className="absolute -bottom-1 -right-1 bg-sky-500 text-white p-1.5 rounded-full shadow-lg border border-white">
        ...
        <button 
          ...
          className="px-4 py-2 hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-xl text-xs font-semibold shrink-0"
        >
        ...
        <form onSubmit={handleUpdateBio} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label htmlFor="prof-headline" className="text-xs font-mono font-bold text-slate-700">Professional Headline</label>
              <input 
                id="prof-headline"
                type="text" 
                value={headline} 
                onChange={(e) => setHeadline(e.target.value)}
                className="w-full text-sm px-3.5 py-2.5 bg-white text-slate-950 rounded-xl border border-slate-300 focus-visible:ring-2 focus-visible:ring-slate-900 focus:outline-none" 
              />
            </div>
            <div className="space-y-1.5">
              <label htmlFor="prof-skills" className="text-xs font-mono font-bold text-slate-700">Skills (Comma separated list)</label>
              <input 
                id="prof-skills"
                type="text" 
                value={skills} 
          ...
          <div className="space-y-1.5">
            <label htmlFor="prof-bio" className="text-xs font-mono font-bold text-slate-700">Student Bio</label>
            <textarea 
              id="prof-bio"
              rows={3}
              value={bio} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-gradient-to-br from-indigo-900 to-slate-950 text-white p-6 rounded-3xl border border-slate-800 shadow-xl space-y-6 relative overflow-hidden">
            ...
            <div className="absolute -bottom-8 -right-8 w-32 h-32 bg-indigo-500/20 blur-2xl rounded-full" />
          </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-slate-500">Student creative showcases in development, design and systems.</p>
              </div>
              <button 
              ...
              <form onSubmit={handleAddPortfolio} className="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-slate-700 flex items-center gap-1">
                    <Sparkles className="h-3.5 w-3.5 text-sky-500" /> Construct Showcase
                  </h4>
                  <button 
                    type="button"
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1.5">
                      ...
                      <select 
                        ...
                        className="w-full text-xs px-2 py-2 rounded-lg border border-slate-200"
                      >
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono font-semibold text-slate-500">Impact Narrative / Explanation</label>
                  <textarea 
                    rows={3}
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
                  <button 
                    ...
                    className="px-3 py-1.5 bg-slate-200 text-slate-700 text-[10px] uppercase font-bold rounded-lg hover:bg-slate-300"
                  >
                    Cancel
                  </button>
                  <button 
            ...
            {profile.portfolio.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {profile.portfolio.map(item => (
                  <div key={item.id} className="rounded-2xl border border-slate-100 overflow-hidden flex flex-col hover:shadow-md hover:border-indigo-100 transition-all bg-white group select-none">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex justify-between items-start gap-1">
                          ...
                          <button 
                            ...
                            className="p-1 rounded text-slate-300 hover:text-red-500 hover:bg-red-50 shrink-0 transition-colors"
                          >
          ...
          {/* Client Reviews evaluation history and Student replies */}
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-1.5">
                  ...
                  <div key={rev.id} className="py-4 space-y-3 first:pt-0 last:pb-0">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div>
                          <p className="text-xs font-bold text-slate-950 font-mono">{rev.reviewerName}</p>
                      ...
                      <div className="space-y-2 pt-1">
                        <div className="flex gap-2">
                          <input 
                            ...
                            className="flex-1 text-[11px] px-3 py-1.5 rounded-lg border border-slate-200 focus:outline-none"
                          />
                          <button 
